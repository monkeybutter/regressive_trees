...
def tree_walk_printer(tree, level=0):
    ...
    if tree.split_var is not None:
        ...
        print level*"  " + "LEAF!:"
        if tree.data.var_limits['date'][0] != float("-inf"):
            print datetime(2012, 1, 1, 0, 0) + timedelta(seconds=365 * 86400.0 * tree.data.var_limits['date'][0] / 360.0)
        if tree.data.var_limits['date'][1] != float("inf"):
            print datetime(2012, 1, 1, 0, 0) + timedelta(seconds=365 * 86400.0 * tree.data.var_limits['date'][1] / 360.0)
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("./web/static/data/synthetic_temp.csv")

    print df.head

    df = df.drop([u'time_of_day', u'day_of_year'], axis=1)
    ...
    var_types_linear = ['linear', 'linear', 'linear']
    var_types_circular = ['linear', 'circular', 'circular']

    data = Data(df, class_var, var_types_circular)
    tree = tree_grower(data, 8000)

    tree_walk_printer(tree)